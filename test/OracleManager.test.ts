...
describe('OracleManager', function () {
  ...
  describe('#registerOracle', () => {
    ...
    it('should register oracle and emit OracleRegistered event', async () => {
      const tx = await oracleManager
        .connect(owner)
        .registerOracle(token0.address, token1.address, chainlinkOracle.address);
      expect(await oracleManager.oracles(token0.address, token1.address)).to.be.equal(
        chainlinkOracle.address,
      );
      expect(await oracleManager.oracles(token1.address, token0.address)).to.be.equal(
        chainlinkOracle.address,
      );
  ...
  describe('#registerStable', () => {
    ...
    it('should register stable and emit StableRegistered event', async () => {
      ...
      expect(await oracleManager.stables(token0.address, token1.address)).to.be.equal(true);
      expect(await oracleManager.stables(token1.address, token0.address)).to.be.equal(true);
  ...
  describe('#removeStable', () => {
    ...
    it('should register stable and emit StableRemoved event', async () => {
      ...
      expect(await oracleManager.stables(token0.address, token1.address)).to.be.equal(false);
      expect(await oracleManager.stables(token1.address, token0.address)).to.be.equal(false);
